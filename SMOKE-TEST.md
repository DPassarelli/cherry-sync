...
- with `--version` it exits `0` and writes a `cherry-sync` version line to stdout; when the caller names the expected version (the release workflow passes the tag), that line must contain it, and
- with `--license` it exits `0` and writes the embedded MIT notice to stdout (both the `MIT License` title and the `THE SOFTWARE IS PROVIDED` warranty paragraph).
...
The `--license` assertion is justified differently, and it is worth being honest about why. Since the notice is embedded at compile time via `go:embed`, and `internal/license`'s unit test pins the embedded copy byte-for-byte against the repository-root `LICENSE`, no plausible *technical* failure reaches the published artifact alone — the version assertion above already proves the artifact was built from the tagged source, so the embed is the tagged source's. The check earns its place on stakes rather than likelihood: now that releases ship a bare executable with no `LICENSE` file beside it, a binary that cannot print its own notice is a license-compliance failure rather than a bug, and one that cannot be retracted once downloaded. Guarding the exact published bytes costs two `grep`s. The assertions are string literals, not a comparison against the repository's `LICENSE`, so the script stays self-contained for the `scp`-to-a-bare-VM path (§7.3); exact equality is the unit test's job, not this script's.
...
- **`_scripts/smoketest.sh <path-to-csync-binary> [expected-version]`** — the assertion runner. Executes the given binary with no arguments, with `--version`, and with `--license`, asserting the §6.1 contract (no-arg: exit `2`, `usage:` on stderr; `--version`: exit `0`, `cherry-sync` line on stdout, and — when `expected-version` is given — that line contains it; `--license`: exit `0`, MIT title and warranty paragraph on stdout). Prints a clear pass/fail line and exits non-zero on failure. Host-agnostic: it does not care whether the binary arrived via download, `scp`, or a local build. A human runs `./_scripts/smoketest.sh ./csync` directly; the release legs additionally pass `${GITHUB_REF_NAME}` to gate version injection.

  *Teeth requirement (house testing rule):* `smoketest.sh` must be verified against both a known-good binary (real `csync` → green) and a degenerate stand-in that violates the contract (e.g. `/bin/true`, which exits `0` and prints nothing csync-shaped → the script must go red). For the version check specifically, verify that a dev-build binary passed an expected release version goes red (the broken-injection class) while the same binary with no expected version stays green. For the license check, build a binary whose `license.Text()` returns `""` and confirm *only* the two `--license` assertions go red; then confirm each fails alone — a binary whose `--license` exits non-zero trips only the exit-code check, and one that prints the title without the warranty paragraph trips only the permission-notice check. Confirm each failure is for the right reason before trusting the gate.
