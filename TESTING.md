...
4. **Wire step definitions.** Bind the scenario's phrases in the step registry and implement them in the `steps_*_test.go` file for the area they exercise, so godog can run it. The test will fail — that's the point.
...
8. **Verify.** Run `go test -count=1 ./...`, `gofmt -l .`, `go vet ./...`, and `gosec ./...` before committing. All four are gates the PR must clear — the first two the human runs, the last two enforced by the lefthook pre-push hook and again in CI. Run them yourself rather than leaning on the hook: a contributor who pushes from an environment without the hook installed (or whose commits are pushed on their behalf from another machine) never triggers it, so the security and vet checks only surface on the PR unless run by hand here. The `-count=1` is not optional after a production change — see [Why `-count=1` matters](#running-tests) below for why a plain `go test ./...` can report a stale pass.
...
Tests assert against what csync produces, but they should not parse it inline in every step. Each `*_parser_test.go` file is the single translation point between one rendered artifact and structured test data: csync's terminal output becomes a `ReportedOutput`, a run log becomes a `ParsedLog`. Add a facade when a new artifact needs asserting against, and the same four rules apply to each.
...
- **One file, one entry point.** A single function is the facade's whole surface: it takes the captured artifact and returns the struct. If rendering changes, that file is the only place that changes.
- **`_test.go` location.** A facade is test-only scaffolding. Putting it in a `_test.go` file means it's compiled out of the production binary and the boundary stays enforced — production code can never call into the parser.
- **Grow the struct as scenarios drive it.** Only add a field when a scenario asserts against it. Pre-building fields for output that doesn't exist yet means the facade is lying about what's available.
...
| `go vet ./...`              | Part of the verify gate; also enforced by lefthook pre-push and CI  |
| `gosec ./...`               | Static security analysis (pinned: `go run github.com/securego/gosec/v2/cmd/gosec@v2.26.1 -quiet ./...`); part of the verify gate, also lefthook pre-push and CI |
